...
void VideoWaves::incIndex()  // call this every frame to calc the offset eeettt
{
...
//--------------------------------------------------------------
void VideoWaves::setup(int _screen_w, int _screen_h) {
    ...
    //int screen_w = 640;
    //int screen_h = 480; //having this be the size of the screen makes for some cool effects
    
    fbo_draw.allocate(screen_w, screen_h);
    fbo_feedback.allocate(screen_w, screen_h);
    syphonTexture.allocate(screen_w, screen_h);
    fbo_blur.allocate(screen_w, screen_h);
...
void VideoWaves::update(){
    //only update if active set a test for that too
    //cam1.update();
}

//--------------------------------------------------------------
void VideoWaves::draw(ofFbo * input_fbo) {
    ...
    shader_mixer.setUniform2f("cam1dimensions",ofVec2f(screen_w,screen_h));
    ...
    if (input_fbo == NULL){
        ...
        shader_mixer.setUniformTexture("cam1",input_fbo->getTexture(),2);
    }
...
//THIS WAS THE VIDEO EAVES KEYPRESS
/*
 if(key=='3'){
 aa=ss=dd=ff=gg=hh=0;
 }
